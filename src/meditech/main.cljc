(ns meditech.main
  "Kotodama WASM entrypoint for the Meditech clean-room actor (L5) — Clojure port.
  ...
  facts (`meditech.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "meditech")
(def tier "L5")
...
(def entity-specs
  [{:entity "Patient"   :plural "patients"   :id-prefix "meditech_pat"
   ...
   {:entity "Observation"      :plural "observations"      :id-prefix "meditech_obs"
    :fields [:resourceType :status :effectiveDateTime :valueString :issued] :required [:resourceType]
    :coerce {}          :refs {}}
   {:entity "Encounter"     :plural "encounters"     :id-prefix "meditech_enc"
    :fields [:resourceType :status :serviceType :period]        :required [:resourceType]
    :coerce {} :refs {}}
   {:entity "MedicationRequest" :plural "medicationrequests" :id-prefix "meditech_med"
    :fields [:resourceType :status :intent :authoredOn]          :required [:resourceType]
    :coerce {}                       :refs {}}
   {:entity "AllergyIntolerance"   :plural "allergyintolerances"   :id-prefix "meditech_all"
    :fields [:resourceType :clinicalStatus :criticality :recordedDate]   :required [:resourceType]
    :coerce {}        :refs {}}
   {:entity "Condition"    :plural "conditions"    :id-prefix "meditech_con"
...
(defn emit-facts
  "EAVT facts for one record: {\"meditech.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "meditech-compat" :tier tier :entities entities} 200])
...
(defn start! [] :meditech-compat/ready)
